...
| 删除追踪 | **不追踪，且增量态下不会自动恢复**：用户删掉同步块后，因 feed 是 watermark 增量（按 `updatedAt` 推进），该高亮不会再被返回，故下次增量同步**不会**重新追加。仅当该高亮在 Acorny 侧被更新（`updatedAt` 上移重新入 feed）或用户重置游标做全量再同步时才会重新出现 | 块属性方案已知局限；避免误导用户以为删了会回来。二期用 tombstone + 可选 full reconciliation/reset |
